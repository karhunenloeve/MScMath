...
\cdots \arrow{r}
  & H_n(\G';A)
        \arrow{r}{H_n((j^{\G}_\bullet)_\ast)}
        \arrow{d}[swap]{H_n\bigl((\Phi'_\bullet)_*\bigr)}
  & H_n(\G;A)
        \arrow{r}{H_n((q^{\G}_\bullet)_\ast)}
...
\cdots \arrow{r}
  & H_n(\Hh';A)
        \arrow{r}{H_n((j^{\Hh}_\bullet)_\ast)}
  & H_n(\Hh;A)
        \arrow{r}{H_n((q^{\Hh}_\bullet)_\ast)}
  & H_n(\Hh/\Hh';A)
...
Throughout, \(\partial_\bullet\) denotes the Moore differential on the integral chain complex \(\bigl(C_c(\G_\bullet,\ZZ),\partial_\bullet\bigr)\), constructed earlier from pushforwards along the face maps of the nerve.
...
0 \arrow{r}
  ...
  & \operatorname{Tor}_1^{\mathbb Z}\bigl(H_{n-1}(\G'),A\bigr)
        \arrow{r}
        \arrow{d}[swap]{\operatorname{Tor}_1^{\mathbb Z}(H_{n-1}(i))}
  & 0
\\
0 \arrow{r}
  & H_n(\G)\otimes_{\mathbb Z} A
  ...
  & \operatorname{Tor}_1^{\mathbb Z}\bigl(H_{n-1}(\G),A\bigr)
        \arrow{r}
        \arrow{d}[swap]{\operatorname{Tor}_1^{\mathbb Z}(H_{n-1}(p))}
  & 0
\\
0 \arrow{r}
  & H_n(\G'')\otimes_{\mathbb Z} A
...
The maps \(H_n(i)\coloneqq H_n(i;A)\) and \(H_n(p)\coloneqq H_n(p;A)\) are induced on homology by the chain maps
\[
i_\bullet^A:C_c(\G'_\bullet,A)\to C_c(\G_\bullet,A),
\qquad
p_\bullet^A:C_c(\G_\bullet,A)\to C_c(\G''_\bullet,A),
\]
obtained by tensoring \(i_\bullet,p_\bullet\) with \(\operatorname{id}_A\) and composing with the chain isomorphisms
...
\begin{proof}~
\begin{enumerate}[noitemsep,nolistsep]
\item This is the failure mechanism established in Corollary~\ref{cor:discrete-necessity} and made explicit in the Cantor-space Example~\ref{ex:phi-iso-and-fails}:
...
Let $\G$ be an ample groupoid, let $A$ be a topological Hausdorff abelian group, and let
...
\begin{enumerate}[noitemsep,nolistsep]
\item $\beta_\bullet\circ \alpha_\bullet=0$ implies
$H_n(\beta_\bullet)\circ H_n(\alpha_\bullet)=0$ for all $n$.

\item $\symbf{\operatorname{im}\bigl(H_n(\alpha_\bullet)\bigr)=\ker\bigl(H_n(\beta_\bullet)\bigr).}$
...
\item $\symbf{\operatorname{im}\bigl(H_n(\beta_\bullet)\bigr)=\ker(\partial_n).}$
...
\item $\symbf{\operatorname{im}(\partial_n)=\ker\bigl(H_{n-1}(\alpha_\bullet)\bigr)}$.
