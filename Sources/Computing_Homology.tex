...
We study Moore homology for pairs of \'etale groupoids with totally disconnected unit spaces. The goal is to compare their homology groups via exact sequences coming from open inclusions and from proper quotient maps.

\begin{definition}\label{def:regular-pair}
Let \(\G\) be an \'etale groupoid and let \(\G'\subseteq \G\) be an open subgroupoid. Set
\(\triangle\coloneqq \G\setminus \G'\).
Equip \(r(\triangle)\subseteq \G_0\) with the final topology induced by \(r\vert_{\triangle}\colon \triangle\to r(\triangle)\), that is,
\(V\subseteq r(\triangle)\) is open if and only if \((r\vert_{\triangle})^{-1}(V)\) is open in \(\triangle\).
\begin{enumerate}[noitemsep,nolistsep]
  \item
  The inclusion \(\G'\subseteq \G\) is called regular if \(r\vert_{\triangle}\colon \triangle\to r(\triangle)\) is an open map for the above topology on \(r(\triangle)\).
  Equivalently, \(r\vert_{\triangle}\) is a quotient map onto its image.
  \item
...
Let \(\G\) be an \'etale groupoid and let \(\G'\subseteq \G\) be an open subgroupoid with the same unit space, \((\G')_0=\G_0\). Set
\(
\triangle \coloneqq \G\setminus \G'
\)
for the complement of \(\G'\) inside \(\G\) on the level of arrows. Thus \(\triangle\) consists exactly of those arrows of \(\G\) that do not belong to \(\G'\). In particular, \(\triangle\cap \G_0=\varnothing\), since all units lie in \(\G'\). Since \(\G'\) is a subgroupoid, it is closed under inversion, hence \(\triangle\) is also closed under inversion: if \(g\in\triangle\) and \(g^{-1}\in \G'\), then \(g=(g^{-1})^{-1}\in \G'\), a contradiction. Consequently, for every \(g\in \triangle\) both endpoints lie in \(r(\triangle)\):
\(
r(g)\in r(\triangle),
s(g)=r(g^{-1})\in r(\triangle).
\)
Equivalently, the subset \(r(\triangle)\subseteq \G_0\) is invariant for \(\triangle\) in the sense that every arrow in \(\triangle\) both starts and ends in \(r(\triangle)\).
...
\(
\Hh_1=\{g\in \G_1\mid r(g),s(g)\in r(\triangle)\},
\)
while \(\Hh'\) has the same unit space \(\Hh'_0=r(\triangle)\) and arrow space
\(
\Hh'_1=\{g\in \G'_1\mid r(g),s(g)\in r(\triangle)\}
=\Hh_1\cap \G'_1.
\)
\end{setting}

In particular, \(\Hh'\) is a subgroupoid of \(\Hh\) with the same unit space. Since every \(g\in\triangle\) satisfies \(r(g),s(g)\in r(\triangle)\), we have \(\triangle\subseteq \Hh_1\). Moreover, for \(h\in \Hh_1\) either \(h\in \G'_1\) or \(h\in \triangle\). If \(h\in \G'_1\), then \(h\in \Hh'_1\) by definition. Hence, on the level of arrows,
\[
\Hh_1=\Hh'_1 \sqcup \triangle,
\qquad
\Hh_0=\Hh'_0=r(\triangle).
\]
This decomposition is the input for a short exact sequence of chain groups built from extension by zero on \(\Hh'_1\subseteq \Hh_1\) and restriction to the complement \(\triangle\). To make this compatible with the \'etale pushforwards in the Moore boundary, one needs a mild regularity condition ensuring that openness in \(\triangle\) is reflected on the unit space through the range map.
...
\item
For every open subset \(U\subseteq \triangle\) and every sequence \((g_k)_k\) in \(\triangle\) converging to some \(g\in \triangle\) with \(r(g)\in r(U)\), one has \(r(g_k)\in r(U)\) for all sufficiently large \(k\).
\end{enumerate}
\end{lemma}
...
Let \(U\subseteq \triangle\) be open and \((g_k)_k\to g\) in \(\triangle\) with \(r(g)\in r(U)\). Then \(g\in r^{-1}(r(U))\cap \triangle\), which is open by \textup{2}. Hence \(g_k\in r^{-1}(r(U))\cap \triangle\) for large \(k\), so \(r(g_k)\in r(U)\) eventually.

\item \textup{\(3.\Rightarrow 2\).}
Fix an open \(U\subseteq \triangle\) and set \(A\coloneqq r^{-1}(r(U))\cap \triangle\). Let \(g\in A\) and let \((g_k)_k\) be a sequence in \(\triangle\) with \(g_k\to g\). Since \(r(g)\in r(U)\), condition \textup{3.} implies \(r(g_k)\in r(U)\) for all sufficiently large \(k\), hence \(g_k\in A\) eventually. Thus \(A\) is sequentially open in \(\triangle\). Since \(\triangle\) is first countable, sequential openness agrees with openness, so \(A\) is open in \(\triangle\).
\end{itemize}
\end{proof}
...
Let \((g_k,h_k)\) be a sequence in \((\triangle\times\triangle)\cap m^{-1}(\G')\) converging to \((g,h)\), and assume that \(gh\in m(V)\).
...
Similarly, \(g_k c_k^{-1}\to gc^{-1}=a\in\triangle\) implies \(g_k c_k^{-1}\in\triangle\) for large \(k\).
Moreover,
\[
(g_k c_k^{-1})(c_k h_k)=g_k h_k,\qquad
c_k h_k\in U_2,\qquad g_k c_k^{-1}\in U_1
\]
for all \(k\) sufficiently large because \(U_1,U_2\) are open and \(a\in U_1\), \(b\in U_2\).
Hence \((g_k c_k^{-1},c_k h_k)\in V\) for large \(k\), and therefore \(g_k h_k\in m(V)\) eventually.
...
As above, \(g_k c_k^{-1}\to a\in\triangle\) gives \(g_k c_k^{-1}\in\triangle\) eventually, and openness of \(U_1,U_2\) yields \(g_k c_k^{-1}\in U_1\), \(c_k h_k\in U_2\) for large \(k\).
Thus \((g_k c_k^{-1},c_k h_k)\in V\) eventually, hence \(g_k h_k\in m(V)\) eventually.
\end{itemize}

In either case, whenever \((g_k,h_k)\to(g,h)\) with \(gh\in m(V)\), we have \(g_k h_k\in m(V)\) for all sufficiently large \(k\).
...
Regularity is designed so that these quotient-type topologies still interact well with the étale structure maps.
...
Then:
\begin{enumerate}[noitemsep,nolistsep]
\item \(\Hh\) and \(\Hh'\) are étale groupoids,
...
It remains to see that \(\Hh\) and \(\Hh'\) are étale.
...
Therefore \(r\) and \(s\) are local homeomorphisms on \(\Hh\) and on \(\Hh'\), so both are étale groupoids.
...
Let \((g_k)_k\) be a sequence in \(\Hh\) and let \(g\in\Hh\).
Then the following conditions are equivalent:
\begin{enumerate}[noitemsep,nolistsep]
\item \(g_k\to g\) in \(\Hh\),
\item \(r(g_k)\to r(g)\) in \(\Hh_0\), \(s(g_k)\to s(g)\) in \(\Hh_0\), and \(g_k\to g\) in \(\G\),
\item \(r(g_k)\to r(g)\) in \(\Hh_0\) and \(g_k\to g\) in \(\G\),
\item \(s(g_k)\to s(g)\) in \(\Hh_0\) and \(g_k\to g\) in \(\G\).
\end{enumerate}
\end{lemma}
...
Assume \(r(g_k)\to r(g)\) in \(\Hh_0\) and \(g_k\to g\) in \(\G\).

\begin{itemize}[noitemsep,nolistsep]
\item \textbf{Case \(g\in \triangle\).}
Let \(U\subseteq\G\) be an open bisection with \(g\in U\).
Then \(g_k\in U\) for all large \(k\).
By Theorem~\ref{thm:H-Hprime-etale}, the map \(r\vert_{\triangle}\colon \triangle\to \Hh_0\) is open, hence \(r(U\cap \triangle)\) is an open neighbourhood of \(r(g)\) in \(\Hh_0\).
Thus \(r(g_k)\in r(U\cap \triangle)\) for all large \(k\).
For such \(k\), there exists a unique \(h_k\in U\cap\triangle\) with \(r(h_k)=r(g_k)\), because \(r\vert_U\colon U\to r(U)\) is a homeomorphism and \(r(U\cap\triangle)=r(U)\cap r(\triangle)\).
Since also \(g_k\in U\) and \(r(g_k)=r(h_k)\), injectivity of \(r\vert_U\) gives \(g_k=h_k\in U\cap\triangle\) for all large \(k\).
...
As in the first case, \(r(U\cap\triangle)\) is an open neighbourhood of \(r(a)=r(g)\) in \(\Hh_0\), so \(r(g_k)\in r(U\cap\triangle)\) for all large \(k\).
For each such \(k\), let \(a_k\in U\cap\triangle\) be the unique element with \(r(a_k)=r(g_k)\).
Then \(a_k\to a\) in \(\Hh\), because \(r\vert_{U\cap\triangle}\colon U\cap\triangle\to r(U\cap\triangle)\) is a homeomorphism and \(r(a_k)=r(g_k)\to r(g)=r(a)\) in \(\Hh_0\).
On \(\triangle\) the \(\Hh\)-topology is the subspace topology from \(\G\), hence \(a_k\to a\) in \(\G\).

Define \(b_k\coloneqq a_k^{-1}g_k\).
By continuity of inversion and multiplication in \(\G\), we have \(b_k\to a^{-1}g=b\) in \(\G\).
Moreover,
\[
r(b_k)=r(a_k^{-1}g_k)=r(a_k^{-1})=s(a_k)\rightarrow s(a)=r(b)
\quad \text{in } \Hh_0,
\]
where the convergence \(s(a_k)\to s(a)\) holds because \(s\) is continuous on the étale groupoid \(\Hh\) and \(a_k\to a\) in \(\Hh\).
...
First, any étale functor \(\varphi:\Hh\to\G\) induces maps on nerves \(\varphi_n:\Hh_n\to\G_n\), hence pushforwards \((\varphi_n)_*\) on compactly supported chains, and these assemble to a chain map by functoriality of pushforward.
...
Assume there is an \'etale functor $\varphi:\Hh\to \G$ which is a groupoid equivalence and such that, for every $n\ge 0$, the induced map on nerves $\varphi_n:\Hh_n\to \G_n$ is a local homeomorphism with finite fibres.
...
For fixed $n$ and $g\in\G_n$ the fibre $\varphi_n^{-1}(g)$ is finite by assumption, hence the sum defining $(\varphi_n)_*\xi$ is finite.
Standard compact-support arguments for pushforward along local homeomorphisms with finite fibres show that $(\varphi_n)_*\xi$ is continuous and compactly supported, so $(\varphi_n)_*$ is a well-defined group homomorphism.
...
Choose an \'etale quasi-inverse $\psi:\G\to\Hh$ for $\varphi$, so that $\psi_n:\G_n\to\Hh_n$ is a local homeomorphism with finite fibres for all $n$.
...
\begin{proof}
By Setting~\ref{setting:subgroupoid} there is an \'etale functor $\Phi:\G\to\Hh$ which restricts to $\Phi':\G'\to\Hh'$ and induces a functor on complements
\[
...
\begin{proposition}[{\cite[Section~7]{putnam2020excision}}]\label{prop:regularity-preserved}
...
\begin{definition}[Reduction map {\cite[Section~4.2]{matui2022long}}]\label{def:reduction-map}
Let \(A\) be a discrete abelian group.
...
\]
and let \(\tau\) be the map induced by \(\varphi_*\vert_{C_c(Y_1,A)}\),
\[
...
so \(\sigma\) and \(\tau\) are induced by \(\varphi_*\).
...
\Pi_2\bigl(\varphi_*f+\pi_2^*C_c(X_2',A)\bigr)&= (\varphi_*f)_{C'}+\pi_2^*C_c(Y_2',A).
\end{aligned}
\]
It suffices to prove
\(
\varphi_*(f_{K'})-(\varphi_*f)_{C'}\in \pi_2^*C_c(Y_2',A).
\)
We prove the stronger identity
\(
\varphi_*(f_{E'})=(\varphi_*f)_{C'}
\)
in \(C_c(Y_2,A)\).
Since \(f_{K'}-f_{E'}\in \pi_1^*C_c(Y_1',A)\), applying \(\varphi_*\) and using the well-definedness computation above yields
\(
\varphi_*(f_{K'})-\varphi_*(f_{E'})\in \pi_2^*C_c(Y_2',A),
\)
...
\)
Thus \(\varphi_*(f_{E'})=(\varphi_*f)_{C'}\) in \(C_c(Y_2,A)\).
\end{itemize}
...
&=\varphi_*(f_{K'})+\pi_2^*C_c(Y_2',A)\\
&=(\varphi_*f)_{C'}+\pi_2^*C_c(Y_2',A)\\
...
Let \(\G\) and \(\G'\) be étale groupoids and let \(\pi:\G\to\G'\) be a continuous homomorphism such that:
\begin{enumerate}[noitemsep,nolistsep]
...
Since \(\G\) and \(\G'\) are étale, the range maps \(r:\G\to\G_0\) and \(r:\G'\to\G_0'\) are local homeomorphisms.
...
Then the groupoids \(\G_{\pi}\) and \(\G'_\pi\) are étale for the topologies induced by the metrics \(d_\pi\) and \(d'_\pi\) from Definition~\ref{def:regular-map}.
\end{proposition}
...
\(r:\G_\pi\to (\G_0)_\pi\).
To see that \(r\) is a local homeomorphism for the \(d_\pi\)-topology, fix \(g\in \G_\pi\) and choose an open bisection \(U\subseteq \G\) with \(g\in U\).
Then \(r|_{U}:U\to r(U)\) is a homeomorphism.
Since \(\pi|_{r(U)}\) is a homeomorphism onto \(\pi(r(U))\) by assumption \textup{3.} and étaleness, the set \(U\cap \G_\pi\) is open in \(\G_\pi\) and \(r(U\cap \G_\pi)=r(U)\cap (\G_0)_\pi\) is open in \((\G_0)_\pi\).
...
Therefore \(\G_\pi\) and \(\G'_\pi\) are étale.
\end{proof}
...
\]
On the other hand,
\((g_1',\dots,g_n')\in (\G'_\pi)_n\) if and only if \(g_1'\in \G'_\pi\), equivalently \(\#\pi^{-1}(g_1')>1\).
Thus
\(
...
\)
Since \(\G'\) is \'etale, \(\varphi\) is a local homeomorphism.
...
The face maps \(d_i:\G_{n}\to \G_{n-1}\) and \(d_i:\G'_{n}\to \G'_{n-1}\) are local homeomorphisms since \(\G\) and \(\G'\) are \'etale, and the squares with \(\pi_n\) commute.
...
Equivalently,
\[
H^n(\G;A)=H^n\Bigl(\Hom_{\ZZ}\bigl(C_c(\G_\bullet,\ZZ),A\bigr)\Bigr).
\]
\end{proposition}
...
Let $\G$ be an ample \'etale groupoid and let $A$ be a discrete abelian group.
...
The UCT for ample \'etale groupoids established above is, in general, a discrete-coefficients result: its proof relies on the identification
\[
...
Conversely, by the standard compact-open partition argument for totally disconnected locally compact Hausdorff spaces, every \(f\in C_c(X,\mathbb Z)\) is a finite \(\mathbb Z\)-linear combination of characteristic functions of pairwise disjoint compact open subsets. Hence elements of \(C_c(X,\mathbb Z)\otimes_{\mathbb Z}A\) are finite sums of tensors \(\chi_U\otimes a\) with \(U\) compact open.
For such a tensor,
\[
...
This is exactly why the UCT for ample \'etale groupoids obtained below is intrinsically a discrete-coefficients statement when homology is defined using compactly supported continuous $A$-valued chains.
...
Thus, for ample \'etale groupoids \(\G\), the universal coefficient theorem in the form of Theorem~\ref{thm:UCT-homology-G}
...
Let \(\G,\G',\G''\) be ample \'etale groupoids.
...
We have thus obtained a homological universal coefficient theorem for ample \'etale groupoids with discrete coefficients.
...
\(C_c(\G_\bullet,\mathbb Z)\).
This yields the standard cohomological universal coefficient short exact sequence
...
\end{enumerate}
Here $x\sim_{\G} y$ means that there exists $g\in \G$ with $s(g)=y$ and $r(g)=x$.
For such $U_1,U_2$ we consider the reductions
\[
\begin{aligned}
\G|_{U_1}&\coloneqq \{g\in \G \mid r(g)\in U_1,\ s(g)\in U_1\},\\
\G|_{U_2}&\coloneqq \{g\in \G \mid r(g)\in U_2,\ s(g)\in U_2\},\\
\G|_{U_1\cap U_2}&\coloneqq \{g\in \G \mid r(g)\in U_1\cap U_2,\ s(g)\in U_1\cap U_2\},
\end{aligned}
\]
...
\begin{proof}
Since $\G$ is ample, it is \'etale, locally compact, Hausdorff, and totally disconnected.
...
For each $k$, the units $r(g_k)$ and $s(g_k)$ lie in the $\G$-orbit of $r(g_1)$, hence belong to $U$ by saturation.
Thus $g_k\in \G|_U$ for all $k$, so $(g_1,\dots,g_n)\in (\G|_U)_n$.
...
\]
are chain complexes.
Equivalently, for all $n\ge 1$ one has
\[
...
\begin{proof}
Since $\G$ is ample, it is \'etale.
By Lemma~\ref{lem:reductions-ample}, each reduction $\G|_{U_1}$, $\G|_{U_2}$, and $\G|_{U_1\cap U_2}$ is ample, hence \'etale.
...
Let $\G$ be an ample groupoid, let $A$ be a Hausdorff topological abelian group, and let
...
\)
so that the standard clopen Mayer--Vietoris argument produces a short exact sequence of Moore chain complexes and hence a long exact sequence in homology. In many applications, however, the relevant chains are compactly supported.
...
\]
Equipped with its standard étale topology -- generated by compact open bisections coming from cylinder sets and restrictions of powers of \(\sigma\) -- \(\mathcal G_A\) is second countable, locally compact, Hausdorff, totally disconnected, and étale; in particular it is ample. This is the same construction of the Deaconu--Renault groupoid of a local homeomorphism as in \cite[§2.5]{ArmstrongBrownloweSims2021}.
